...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        B_, N, C = x.shape
        # print(x.shape)  torch.Size([4096, 49, 96])
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # print(qkv.shape)  torch.Size([3, 4096, 3, 49, 32])
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        q = q * self.scale
        # print(q.shape)  torch.Size([4096, 3, 49, 32])
        attn = (q @ k.transpose(-2, -1))
        # print(attn.shape)  torch.Size([4096, 3, 49, 49])

        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        # print(relative_position_bias.shape)  torch.Size([49, 49, 3])
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        # print(relative_position_bias.shape)  torch.Size([3, 49, 49])
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        # print(x.shape)  # torch.Size([64, 3136, 96]), torch.Size([64, 784, 192]), torch.Size([64, 196, 384]), torch.Size([64, 49, 768])
        x = x.view(B, H, W, C)
        # print(x.shape)  # torch.Size([64, 56, 56, 96]), torch.Size([64, 28, 28, 192]), torch.Size([64, 14, 14, 384]), torch.Size([64, 7, 7, 768])
        ...
        # print(x_windows.shape)  # torch.Size([4096, 7, 7, 96]) each window contains 7 grids, 8 windows each row/column
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        # print(x_windows.shape)  # torch.Size([4096, 49, 96])

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        # print(attn_windows.shape)  # torch.Size([4096, 49, 96])

        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        # print(attn_windows.shape)  # torch.Size([4096, 7, 7, 96])
        shifted_x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C
        # print(shifted_x.shape)  # torch.Size([64, 56, 56, 96])
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)  # torch.Size([64, 3136, 96]) # 56*56 patchs, 4*4*3 -> 96
        # print(x.shape)
        if self.absolute_pos_embed is not None:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        x = self.layers(x)
        # print(x.shape)  torch.Size([64, 49, 768])
        x = self.norm(x)  # B L C
        x = self.avgpool(x.transpose(1, 2))  # B C 1
        # print(x.shape)  torch.Size([64, 768, 1])
        x = torch.flatten(x, 1)
        # print(x.shape)  torch.Size([64, 768])
        return x
