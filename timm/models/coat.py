...
class CoaT(nn.Module):
    ...
    def forward_features(self, x0):
        B = x0.shape[0]
        # print(x0.shape)  torch.Size([32, 3, 224, 224])

        # Serial blocks 1.
        x1 = self.patch_embed1(x0)
        # print(x1.shape, self.patch_embed1.grid_size)  # torch.Size([32, 3136, 152]) (56, 56)  56*56 patchs, 4*4*3->152
        H1, W1 = self.patch_embed1.grid_size
        x1 = self.insert_cls(x1, self.cls_token1)
        # print(x1.shape)  # torch.Size([32, 3137, 152])
        for blk in self.serial_blocks1:
            x1 = blk(x1, size=(H1, W1))
        # print(x1.shape)  torch.Size([32, 3137, 152])
        x1_nocls = self.remove_cls(x1)
        # print(x1_nocls.shape)  torch.Size([32, 3136, 152])
        x1_nocls = x1_nocls.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()
        # print(x1_nocls.shape)  torch.Size([32, 152, 56, 56])

        # Serial blocks 2.
        x2 = self.patch_embed2(x1_nocls)
        # print(x2.shape, self.patch_embed2.grid_size)  torch.Size([32, 784, 216]) (28, 28)
        H2, W2 = self.patch_embed2.grid_size
        x2 = self.insert_cls(x2, self.cls_token2)
        # print(x2.shape)  torch.Size([32, 785, 216])
        for blk in self.serial_blocks2:
            x2 = blk(x2, size=(H2, W2))
        # print(x2.shape)  torch.Size([32, 785, 216])
        x2_nocls = self.remove_cls(x2)
        # print(x2_nocls.shape)  torch.Size([32, 784, 216])
        x2_nocls = x2_nocls.reshape(B, H2, W2, -1).permute(0, 3, 1, 2).contiguous()
        # print(x2_nocls.shape)  torch.Size([32, 216, 28, 28])

        # Serial blocks 3.
        x3 = self.patch_embed3(x2_nocls)
        H3, W3 = self.patch_embed3.grid_size
        # print(x3.shape, self.patch_embed3.grid_size)  # torch.Size([32, 196, 216]) (14, 14)
        x3 = self.insert_cls(x3, self.cls_token3)
        for blk in self.serial_blocks3:
            x3 = blk(x3, size=(H3, W3))
        # print(x3.shape)  # torch.Size([32, 197, 216])
        x3_nocls = self.remove_cls(x3)
        x3_nocls = x3_nocls.reshape(B, H3, W3, -1).permute(0, 3, 1, 2).contiguous()
        # print(x3_nocls.shape)  torch.Size([32, 216, 14, 14])
        ...
        for blk in self.parallel_blocks:
            ...
            # torch.Size([32, 3137, 152]) torch.Size([32, 785, 216]) torch.Size([32, 197, 216]) torch.Size([32, 50, 216])
