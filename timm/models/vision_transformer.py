...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # print(x.shape)  # torch.Size([128, 197, 384])
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # 3, B, N_heads, N_tokens, N_dim/N_heads
        # print([item.shape for item in qkv])  # [torch.Size([128, 6, 197, 64]), torch.Size([128, 6, 197, 64]), torch.Size([128, 6, 197, 64])]
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        # print(attn.shape)  # torch.Size([128, 6, 197, 197]) patch-level attention
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape, self.attn(self.norm1(x)).shape)  # torch.Size([128, 197, 384]) torch.Size([128, 197, 384])
        x = x + self.drop_path(self.attn(self.norm1(x)))
        # print(x.shape, self.mlp(self.norm2(x)).shape)  # torch.Size([128, 197, 384]) torch.Size([128, 197, 384])
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # print(x.shape)  # torch.Size([128, 196, 384]), 196=14*14, 16*16*3->384. N_token=196
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        # print(self.cls_token.shape, cls_token.shape)  # torch.Size([1, 1, 384]) torch.Size([128, 1, 384])
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)
        else:
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        # print(x.shape)  # torch.Size([128, 197, 384]). N_token=197
        x = self.pos_drop(x + self.pos_embed)
        # print(x.shape)  # torch.Size([128, 197, 384])
        x = self.blocks(x)
        # print(x.shape)  # torch.Size([128, 197, 384])
        x = self.norm(x)
        # print(x.shape, self.pre_logits(x[:, 0]).shape)  # torch.Size([128, 197, 384]) torch.Size([128, 384])
        if self.dist_token is None:
            return self.pre_logits(x[:, 0])
            # get the No.0 classification token
        else:
            return x[:, 0], x[:, 1]

    def forward(self, x):
        x = self.forward_features(x)
        # print(x.shape)  # torch.Size([128, 384])
        if self.head_dist is not None:
            ...
            x = self.head(x)
            # print(x.shape)  # torch.Size([128, 1000])
        return x
